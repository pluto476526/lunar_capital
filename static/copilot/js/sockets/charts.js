      ...
      heatmap: {
        ...
        colorScale: {
          ...
          ranges: [
            ...
            // { from: -0.75, to: -0.5, color: colors.paletteFill[1], name: 'Strong Negative' },
            ...
            { from: -1, to: -0.6, color: colors.palette[1] },
    { from: -0.6, to: -0.2, color: colors.palette[2] },
    { from: -0.2, to: 0.2, color: colors.grid },
    { from: 0.2, to: 0.6, color: colors.paletteFill[0] },
    { from: 0.6, to: 1, color: colors.palette[0] }
          ]
        }
      }
    },
    ...
    annotations: {
      yaxis: [
        ...
        },
        {
          y: 50,
          y2: 100,
          fillColor: isDarkMode() ? colors.paletteFill[0] : colors.paletteFill[0],
          opacity: 0.2,
          label: {
            text: "Bullish Zone",
            style: {
              color: colors.palette[0],
              background: isDarkMode() ? colors.paletteFill[0] : colors.paletteFill[0],
              fontSize: '12px'
            }
          }
        },
        {
          y: 0,
          y2: 50,
          fillColor: isDarkMode() ? colors.paletteFill[1] : colors.paletteFill[1],
          opacity: 0.2,
          label: {
            text: "Bearish Zone",
            style: {
              color: colors.palette[1],
              background: isDarkMode() ? colors.paletteFill[1] : colors.paletteFill[1],
              fontSize: '12px'
            }
          }
        }
...
function renderStockCorrelationMatrix(element, sectorCorrelationMatrix) {
    ...
    const options = {
        ...
        yaxis: {
            ...commonChartConfig.yaxis,
            labels: {
                ...commonChartConfig.yaxis.labels,
                style: {
                    ...commonChartConfig.yaxis.labels.style,
                    fontSize: '11px'
                }
            }
        },
