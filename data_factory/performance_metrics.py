...
class TradingPerformanceAnalyzer:
    def __init__(
        self, csv=None, initial_capital=10000, base_currency="GBP", report_data=None
    ):
        """
    ...
    def preprocess_data(self):
        ...
        self.df["date"] = pd.to_datetime(
            self.df["Transaction Date"], dayfirst=True, errors="coerce"
        )
        ...
        # Numeric conversions
        self.df["quantity"] = pd.to_numeric(self.df["Amount"], errors="coerce").fillna(
            0
        )
        ...
        self.df["balance"] = pd.to_numeric(self.df["Balance"], errors="coerce").fillna(
            0
        )
        self.df["currency"] = self.df["Currency"].astype(
            str
        )  # Ensure currency is string

        # Check for non-numeric values in critical columns
        if self.df["closing_price"].isna().any():
            self.df["closing_price"] = self.df["closing_price"].fillna(
                0
            )  # Fill NaN with 0 for trade_value
    ...
    def convert_to_base_currency(self):
        ...
        def compute_rate(currency):
            if currency == self.base_currency:
                return 1.0
            return fx_rates_numeric.get(currency, 1.0) / fx_rates_numeric.get(
        ...
        self.df["balance_base"] = (
            self.df["balance"].fillna(0) * self.df["conversion_rate"]
        )
        self.df["trade_value_base"] = (
            self.df["trade_value"].fillna(0) * self.df["conversion_rate"]
        )
    ...
    def calculate_metrics(self):
        ...
        # Expectancy
        self.expectancy = (self.win_rate * self.average_win) - (
            (1 - self.win_rate) * abs(self.average_loss)
        )
