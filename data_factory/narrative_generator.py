...
class BaseNarrativeGenerator:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are an expert financial market analyst specializing in {self.asset_class}. Provide concise, professional analysis based on the provided data.",
                    },
    ...
    def _generate_detailed_narrative(
        self, market_context: Dict, news_analysis: Dict
    ) -> str:
...
class ForexNarrativeGenerator(BaseNarrativeGenerator):
    ...
    def _extract_news_themes(self, news_items: List[Dict]) -> List[str]:
        ...
        top_themes = sorted(
            [(k, v) for k, v in key_themes.items()], key=lambda x: x[1], reverse=True
        )[:3]
    ...
    def create_llm_prompt(
        ...
        news_str = (
            f"News sentiment: {news_analysis.get('overall_sentiment', 'neutral')}. "
        )
        ...
        try:
            ...
            if self.use_llm:
                ...
                if llm_response:
                    ...
                    headline = (
                        parts[0].replace("Headline:", "").strip()
                        if len(parts) > 0
                        else "Forex Market Update"
                    )
...
class CryptoNarrativeGenerator(BaseNarrativeGenerator):
    ...
    def generate_market_dominance(self, market_data: Dict) -> Dict[str, Any]:
        ...
        for symbol, data in market_data.get("symbols", {}).items():
            ...
            if base_symbol == "BTC":
                ...
                btc_data = {"price": current_price, "change": price_change}
    ...
    def create_llm_prompt(
        self, market_context: Dict, news_analysis: Dict, dominance_analysis: Dict
    ) -> str:
