...
def format_asset_data(
    api_response: Dict[str, Any], symbols: List[str]
) -> Dict[str, Any]:
    """
    ...
    if not isinstance(api_response, dict):
        logger.error(
            f"Invalid api_response type: expected dict, got {type(api_response)}"
        )
...
def calculate_atr(
    highs: np.ndarray, lows: np.ndarray, closes: np.ndarray, period: int = 10, **kwargs
) -> float:
    ...
    return safe_talib_calculation(
        talib.ATR, 0.0, highs, lows, closes, timeperiod=period, **kwargs
    )
...
def calculate_bollinger_bands(
    prices: np.ndarray, period: int = 20, nbdev: int = 2, **kwargs
) -> Tuple[float, float, float]:
    ...
    try:
        ...
        upper, middle, lower = talib.BBANDS(
            prices, timeperiod=period, nbdevup=nbdev, nbdevdn=nbdev, **kwargs
        )
        return (
...
def calculate_adx(
    ...
    return safe_talib_calculation(
        talib.ADX, 0.0, highs, lows, closes, timeperiod=period, **kwargs
    )
...
def calculate_opening_range(df: pd.DataFrame, timeframe: str) -> Tuple[float, float]:
    ...
    or_high = (
        first_bars["high"].max()
        if "high" in first_bars.columns
        else first_bars["close"].iloc[0]
    )
    or_low = (
        first_bars["low"].min()
        if "low" in first_bars.columns
        else first_bars["close"].iloc[0]
    )
...
def calculate_intraday_metrics(df: pd.DataFrame, timeframe: str) -> Dict[str, Any]:
    ...
    intraday_range_pct = (
        ((intraday_high - intraday_low) / current_price * 100)
        if current_price != 0
        else 0
    )
    ...
    avg_vol = (
        np.mean(volumes[-vol_window:])
        if vol_window > 0
        else (volumes[-1] if len(volumes) > 0 else 1)
    )
    ...
    open_price = (
        ...
        else closes[0] if len(closes) > 0 else 0
    )
    intraday_change_pct = (
...
def calculate_technical_indicators(
    data: List[Dict], symbol: str, timeframe: str = "1h", rsi_period: int = 9, **kwargs
) -> Dict[str, Any]:
    ...
    sma_5 = (
        np.mean(closes[-5:]) if len(closes) >= 5 else current_price
    ...
    bb_upper, bb_middle, bb_lower = calculate_bollinger_bands(
        closes, period=bb_period, **kwargs
    )  # Volatility bands
    atr_period = max(5, int(10 * period_mult))
    atr = calculate_atr(
        highs, lows, closes, period=atr_period, **kwargs
    )  # Volatility measure
    adx = calculate_adx(
        highs, lows, closes, period=atr_period, **kwargs
    )  # Trend strength
    ...
    trend_direction = (
        "bullish" if current_price > sma_10 else "bearish"
    )  # Price relative to short-term MA
    trend_strength = (
        min(0.99, abs(current_price - sma_10) / sma_10) if sma_10 != 0 else 0
    )  # How strong is the trend

    # Support/resistance levels - key price levels where reversals may occur
    lookback = min(10, len(highs))
    support_level = (
        np.min(lows[-lookback:]) if lookback > 0 else current_price
    )  # Recent low as support
    resistance_level = (
        np.max(highs[-lookback:]) if lookback > 0 else current_price
    ...
    avg_volume = (
        np.mean(volumes[-vol_lookback:])
        if vol_lookback > 0
        else volumes[-1] if len(volumes) > 0 else 0
    )
    volume_ratio = (
    ...
    pivot_points = calculate_standard_pivot_points(
        highs, lows, closes
    )  # Key intraday levels
    ...
    or_breakout = (
        ...
        else ("down" if current_price < or_low else "inside")
    )  # Breakout status
...
def append_breadth_series(
    breadth_pct: float, maxlen: int = 20, key: str = "breadth_series"
) -> List[float]:
...
def get_session_activity(asset_data: Dict[str, Any]) -> str:
    ...
    for symbol, data in asset_data.get("symbols", {}).items():
        ...
        latest = vols[-1]
        avg_past = (
            float(np.mean(vols[-6:-1])) if len(vols) > 5 else float(np.mean(vols[:-1]))
        )
        if avg_past > 0:
...
def calculate_market_liquidity(asset_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for symbol, data in asset_data["symbols"].items():
        ...
        if "close" in df.columns and len(df) > 5:
            closes = df["close"].values
            price_volatility = (
                np.std(closes[-5:]) / np.mean(closes[-5:])
                if np.mean(closes[-5:]) > 0
                else 0
            )
            stability_factor = 1 / (
                1 + price_volatility
            )  # Higher stability = higher liquidity score
...
def get_top_movers(
    ...
    if timeframe in ["1m", "5m", "15m", "30m", "1h"]:
        valid = valid[
            valid["volume_ratio"] > 1.0
        ]  # Only include moves with above-average volume
    ...
    cache_set(
        f"top_movers:{metric}:{top_n}", result, ttl=30
    )  # Brief cache for frequently changing data
    return result
...
def calculate_volatility_index(asset_data: Dict[str, Any], period: int = 10) -> float:
    ...
    vol = (
        round(float(np.mean(vals)), 1) if vals else 0.0
    )  # Market-wide volatility average
    cache_set("volatility_index", vol, ttl=_CACHE_TTL)
    return vol


def technical_breadth_summary(
    asset_data: Dict[str, Any], asset_class: str = None
) -> Dict[str, Any]:
...
def calculate_correlation_matrix(asset_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        corr_matrix = df_prices.corr().round(
            2
...
def calculate_index_returns(
    asset_data: Dict[str, Any], index_symbols: List[str]
) -> Dict[str, Any]:
...
def get_categorized_news(
    raw_news: Dict[str, Any], ttl: int = _CACHE_TTL
) -> Dict[str, list[dict]]:
    ...
    if categorized is not None:
        return categorized
    try:
        categorized = news_processor.categorize_news(
            raw_news
        )  # External news processing
...
def get_asset_overview(
    symbol: str, asset_data: Dict[str, Any], asset_class: str, timeframe: str = "1m"
) -> Dict[str, Any]:
    ...
    technicals = calculate_technical_indicators(
        symbol_data, symbol, timeframe=timeframe
    )
    ...
    technicals.update(
        {
            "market_status": market_data.get(
                "market_status", "Unknown"
            ),  # Overall market condition
            "volatility_index": market_data.get(
                "volatility_index", 0
            ),  # Market volatility context
            "current_session": market_data.get(
